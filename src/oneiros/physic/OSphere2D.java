...
public class OSphere2D {
     ...
     */
    public OSphere2D(OPoint pCenter) {
        this.location = new OVector2D(pCenter);
        this.speed = new OVector2D(0, 0);
    }
     ...
     */
    private double distanceFrom(OSphere2D pSphere) {
     ...
     *         <code>false</code> altrimenti.
     */
    public boolean isTouching(OSphere2D pSphere) {
     ...
     */
    public void collideWith(OWall pWall) {
     ...
     */
    public void collideWith(OSphere2D pSphere) {
        ...
        OPoint M = new OPoint((this.location.x + pSphere.location.x) / 2, (this.location.y + pSphere.location.y) / 2);

        this.location.setDistanceFrom(M, OSphere2D.BALL_SIZE / 2);
        pSphere.location.setDistanceFrom(M, OSphere2D.BALL_SIZE / 2);
    }
}
