 ...
 * @author Oneiros
 */
public class OWall extends java.awt.geom.Line2D.Double {
     ...
     */
    public OWall(OPoint pA, OPoint pB) {
        super(pA, pB);
    }
     ...
     */
    public double distanceFrom(OSphere2D pBall) {
     ...
     */
    public boolean isTouching(OSphere2D pBall) {
        return this.distanceFrom(pBall) < OSphere2D.BALL_SIZE / 2 || this.getPenetration(pBall) > 0;
    }
     ...
     */
    public double getPenetration(OSphere2D pBall) {
        double α = this.getAngle();
        OPoint a = new OPoint(x1, y1);
        OPoint b = new OPoint(x2, y2);
        OPoint c = new OPoint(pBall.location.x, pBall.location.y);

        a.rotate(α);
        b.rotate(α);
        c.rotate(α);

        if (c.x > a.x && c.x < b.x && c.y >= (a.y - OSphere2D.BALL_SIZE / 2)) {
            return Math.abs(c.y - (a.y - OSphere2D.BALL_SIZE / 2));
        } else {
            return 0;
        }
    }
     ...
     * @param pBall La sfera penetrata nel muro
     */
    public void moveAway(OSphere2D pBall) {
        double α = this.getAngle();
        OPoint a = new OPoint(this.x1, this.y1);
        OPoint b = new OPoint(this.x2, this.y2);
        OPoint c = new OPoint(pBall.location.x, pBall.location.y);
        ...
        double aux = OSphere2D.BALL_SIZE / 2;
    ...
    public double getAngle() {
        return new OPoint(this.x1, this.y1).getAngularCoordFromPoint(new OPoint(this.x2, this.y2));
    }
}
